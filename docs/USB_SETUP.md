...
`deploy.args` block reads optional per-USB parameters (see
[DEPLOY_ARGS.md](DEPLOY_ARGS.md)) and is required if you plan to use
`build_iso.ps1` or per-USB args files. Keep this in sync with
`scripts/build_boot_wim.ps1`'s `$startnet` here-string — that file
