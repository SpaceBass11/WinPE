...
## Phase 7 deep-dive findings and fixes
- Added explicit readonly/write-protection failure detection in `Invoke-Diskpart` by parsing diskpart output for `write protected` / `read-only` signatures.
- Added targeted recovery hints for readonly disks to avoid generic failure output when `diskpart` fails for protection reasons.
