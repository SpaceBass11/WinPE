...
## 2026-06-14 — Masterize CI check #19 covers all five typed-confirmation phrases

**Investigated:** 30 open routine PRs (#65-#94, all generated by prior
autonomous-maintenance passes). Surveyed which areas already have
in-flight work to avoid duplication. `build_iso.ps1` safety has 7+
open PRs, `prepare_wim.ps1` has 2, Pester coverage gaps have 4
(`-Silent` foundationals #69, `Test-FinalWipeConfirmation` parser
#75, `-DataDiskNumber`/extra-wipe overlap #81, `Initialize-BitLockerSetup`
generated script #71), the typed-confirmation chain docs drift has
#92 (AGENTS.md + PR template only — does not touch CI), and the
SCRIPT_REFERENCE.md version resync has #94. The `Show-ImageList` /
`Show-ImageSelection` refactor (deferred ~7 routine entries) is
PR #56. CI workflow itself has only #68 (masterize check #24
`set/p` vs `for/f` deploy.args regex) and #71 (adds the new
`test_bitlocker_setup.ps1` step) touching it — neither overlaps
masterize check #19.

**Found:** Masterize Phase 1B check #19 ("Safety confirmation
strings present") only guards `'DELETE ALL DATA'` and
`'DESTROY SYSTEM'`. The deploy script uses **five** user-facing
typed-confirmation phrases:

- `'ERASE'` — final target-disk confirmation (lines 762, 808)
- `'DESTROY SYSTEM'` — system-disk override (lines 741, 755, 796)
- `'CONTINUE ANYWAY'` — escape the WinPE skip-warning (line 537-8)
- `'WIPE ALL'` — additional-wipe disk batch (lines 1460, 1462)
- `'WIPE DATA'` — data-disk format confirmation (lines 1722, 1812-3)

Plus `'DELETE ALL DATA'` as the legacy accept-string in
`Test-FinalWipeConfirmation` (line 716). A refactor that silently
dropped any of `ERASE` / `CONTINUE ANYWAY` / `WIPE ALL` / `WIPE DATA`
would weaken the corresponding destructive-operation gate without
CI noticing — and the masterize playbook describes itself as the
"mechanical doc-consistency and code-safety checks" backstop.
PR #92 ("docs: resync typed-confirmation chain in AGENTS.md +
PR template") fixes the docs-side drift for the same five phrases
but does not touch the CI gate. This entry closes the script-side
half.

**Changed:**
- `.github/workflows/ci.yml` — extended the `for s in ...` loop in
  check #19 from 2 phrases to 6. Updated the check name and added a
  comment block naming each phrase's purpose. Order matches the
  typed-confirmation chain table in `README.md` (which is already
  the source of truth for these phrases) so a future reader can
  cross-check at a glance.
- `CHANGELOG.md` — bullet under `## Unreleased / ### Changed`
  describing the gate extension and the regression class it now
  catches. No version bump (CI-only change).

**Verification:**
- `pwsh` 7.4.6 installed per the CLAUDE.md bootstrap recipe
  (PowerShell/Releases tarball into `/opt/pwsh/`).
- `tests/test_parse.ps1` → 48/0 (unchanged baseline; CI-config
  change, deploy script untouched).
...
- YAML well-formedness: `python3 -c 'import yaml;
  ...
  6 jobs (`syntax`, `pester`, `pssa`, `actionlint`, `link-check`,
  `masterize`) still present.
- **Gate-on-current-main check** (the critical one — the new
  assertions must already be satisfied so CI stays green at merge):
  ran the extended `for s in 'ERASE' 'DESTROY SYSTEM'
  'CONTINUE ANYWAY' 'WIPE ALL' 'WIPE DATA' 'DELETE ALL DATA'`
  loop locally against `unified_winpe_deploy.ps1` → all six
  present, `ok=1`.
- **Mutation test** (confirms the gate fails on a real regression):
  ran `sed "s/'WIPE DATA'/'WIPE-DATA'/g"` on a copy, re-ran the
  loop → `missing: 'WIPE DATA'`, `ok=0`. The gate would have
  shipped silent green before this change.

**Risks / follow-ups:**
- Minimal. CI-only change; no deploy-script, test, or doc behavior
  altered. The new assertions pass on current main, so no
  prerequisite PR has to land first (unlike the
  `SCRIPT_REFERENCE.md` version gate that PR #94's follow-up
  suggests — that one would fail until #94 lands).
- The `grep -qF` pattern is loose: `'ERASE'` also matches `'ERASED!'`
  log strings (lines 692, 1412). A pathological refactor that
  dropped every typed-prompt occurrence of `'ERASE'` but kept
  the `'ERASED!'` markers would still pass. Tightening to
  `grep -qE "Type '?ERASE'?"` would overfit to today's exact
  prompt wording. The existing check uses the same loose form
  (`grep -qF 'DELETE ALL DATA'`), so the new lines stay
  consistent with the surrounding gate style. Strictly more
  coverage than before; not a regression.
- Cross-doc follow-up: README.md already enumerates all five
  phrases (lines 390-393); AGENTS.md and PR_TEMPLATE.md will
  too once #92 merges. A future masterize check could grep
  those three docs for the same five phrases and fail if any
  drifts, but doing so before #92 lands would fail on main.
  Punted to a later pass.
- Coordination with open PRs:
  - **PR #68** (masterize check #24 deploy.args regex) — touches
    a different check block in the same YAML; trivial merge.
  - **PR #71** (adds `test_bitlocker_setup.ps1` step to the
    `syntax` job) — touches a different job entirely; no overlap.
  - **PR #92** (docs side of this exact drift class) — different
    files; complementary. Whichever lands first, the other rebases
    trivially.
