...
- **PowerShell 5.1+ compatible** (MDT runs scripts via `cscript`/`wscript` wrappers). The admin workstation runs Windows PowerShell 5.1 -- NOT PowerShell 7. Do not use PS7-only syntax. Known incompatibilities to avoid:
  - `return if (...) { x } else { y }` -- PS7 only. Use `if ($v) { return $x } else { return $y }`.
  - `??` null-coalescing operator -- PS7 only. Use `if ($x -eq $null) { $y } else { $x }`.
  - `?.` null-conditional member access -- PS7 only.
  - Ternary `condition ? a : b` -- PS7 only. Use `if`/`else`.
  - `foreach` parallel `-Parallel` parameter on `ForEach-Object` -- PS7 only.
  - Pipeline chain operators `&&` and `||` -- PS7 only.
