...
| `tests/test_wim_parser.ps1` | Fixture test for the `dism /Get-WimInfo` regex parser (guards `Get-WimImageInfo` against silent edition mis-attribution) |
| `tests/test_disk_enumeration.ps1` | Fixture test for `Get-SystemDisks` (USB/removable filter + Linux/LVM partition-count rendering) |
...
| `tests/test_disk_enumeration.ps1` | Fixture test for `Get-SystemDisks` — USB/removable/CD-ROM filter and the `Win32_DiskDrive.Partitions` source-of-truth that keeps Linux/LVM disks from being reported as empty in the TUI | Anywhere with `pwsh` (also CI) |
